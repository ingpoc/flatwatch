#!/bin/bash
# restart-servers.sh - Restart FlatWatch development servers
#
# Stack: Next.js frontend (3000), FastAPI backend (8000)
#
...
echo "=== Restarting FlatWatch Servers ==="
...
if [ "$NO_BACKEND" = false ]; then
  ...
  if [ -d "backend" ]; then
    ...
    echo "⚠ backend/ directory not found, skipping backend" >&2
  fi
fi
...
if [ "$NO_FRONTEND" = false ]; then
  ...
  if [ -d "frontend" ]; then
    ...
    echo "⚠ frontend/ directory not found, skipping frontend" >&2
  fi
fi
...
echo ""
echo "=== FlatWatch servers running ==="

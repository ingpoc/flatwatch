...
const WALLET_ADDRESS = 'wallet-fixture';
...
function trustSurface(state: IdentityPresentTrustState): TrustSurface {
  return {
    ...
    state_reason: `fixture reason for ${state}`,
    verifications:
              ...
              decision:
                state === 'verified'
                  ...
                  : state === 'manual_review'
                    ? 'manual_review'
                    : 'reject',
              reason: `fixture verification reason for ${state}`,
            },
          ],
  };
}
...
describe('fetchTrustSnapshot', () => {
  ...
  it('returns no_identity without a subject and does not call the trust API', async () => {
    const snapshot = await fetchTrustSnapshot('');
    ...
    expect(snapshot.reason).toContain('Authenticate with AadhaarChain');
  ...
  it.each(TRUST_STATES)('maps AadhaarChain fixture state %s', async (state) => {
    ...
    expect(snapshot.reason).toBe(`fixture reason for ${state}`);
